...
int distanciaDaRaiz(struct sNODE *no, int dado) {
    ...
    else if (dado > no->dado && no->dir != NULL) return 1 + distanciaDaRaiz(no->dir, dado);
    else if (dado < no->dado && no->esq != NULL) return 1 + distanciaDaRaiz(no->esq, dado);
    else return 0;
}
